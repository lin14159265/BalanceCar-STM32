...
//    while(lock==1);        // 初始化锁定（被注释）
    ...
    /* 显示初始化 */
    OLED_ShowString(1, 1, "jiao du:");  // 第3行显示"角度："
    OLED_ShowString(2, 1, "ju li:");    // 第4行显示"距离："
    OLED_ShowString(3, 1, "sd:");       // 第5行显示"速度："
...
//        Length = Senor_Using(); // 获取超声波测距结果
        /* 数据显示更新 */
        OLED_ShowSignedNum(1, 9, Pitch*100, 5);        // 显示俯仰角
//        OLED_Float(4, 64, Length, 1);       // 显示距离值
        OLED_ShowSignedNum(4, 3, Encoder_Left,5);      // 显示左编码器值（第5行第8列）
        OLED_ShowSignedNum(4, 11, Encoder_Right,5);     // 显示右编码器值（第6行第8列）
    }
}
