...
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t Decimals) 
{
    ...
    decimal_part = decimal_part * decimal_factor + 0.5f;
    uint32_t decimal_digits = (uint32_t)decimal_part;

    // 处理进位（例如 0.999 → 1.00）
    if (decimal_digits >= decimal_factor) 
		{
        integer_part++;
        decimal_digits = 0;
    }
    ...
    // 显示整数部分
    uint8_t num_str[12]; // 缓冲区，最多支持10位整数
    uint8_t idx = 0;
    ...
    if (temp == 0)	
		...
		{
        while (temp > 0 && idx < sizeof(num_str)) 
				{
    ...
    for (uint8_t i = 0; i < Decimals; i++) 
		{
        uint8_t digit = decimal_digits / OLED_Pow(10, Decimals - i - 1) % 10;
