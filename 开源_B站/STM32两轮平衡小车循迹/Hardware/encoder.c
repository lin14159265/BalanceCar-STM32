...
void Encoder_Z_Init(void){//左编码器初始化(PA6,PA7)
	/*开启时钟*/
...
int16_t Encoder_Z_Get(void){//获取左编码器的增量值
...
void Encoder_Y_Init(void){//右编码器初始化(PB6,PB7)
	/*开启时钟*/
...
int16_t Encoder_Y_Get(void){//获取右编码器的增量值
	/*使用Temp变量作为中继，目的是返回CNT后将其清零*/
	int16_t Temp;
